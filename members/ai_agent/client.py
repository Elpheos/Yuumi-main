...
- REGLE IMPORTANTE - VARIER LE STYLE NE VEUT PAS DIRE EXCLURE LE TYPE DE COMMERCE NOMME : si la demande nomme un type de commerce ou de produit precis (ex: "burger", "tacos", "kebab") tout en demandant quelque chose de "plus original", "moins classique", ou "different", NE PAS EXCLURE la categorie correspondante par reflexe - un commerce de cette categorie peut tres bien etre la reponse "originale" recherchee (ex: un fast-food atypique, peu connu, ou avec un concept different des chaines classiques). Inclus cette categorie EN PLUS d'autres categories pertinentes si la demande peut aussi s'interpreter plus largement - ne la remplace pas systematiquement par des categories totalement differentes (ex: restaurants traditionnels) sauf si la demande le justifie clairement. Utilise ton jugement : le but est de ne pas rater le commerce original qui existe justement dans la categorie nommee, pas d'imposer une seule interpretation rigide.
...
def recommend_stores(user_query, stores_list, store_ids_par_produit=None,
    ...
    system_prompt = (
        ...
        "verifie que ce commerce vend reellement le produit demande. "
        "Ne JAMAIS attribuer confiance='confirme' sur la seule base de la "
        "description ou de la categorie, meme si elle te semble tres "
        "explicite - sans le marqueur [CONFIRME], c'est toujours une "
        "deduction, jamais une certitude verifiee.\n"
        "   - confiance='deduit' dans tous les autres cas, y compris quand "
        "la description du commerce semble tres pertinente. C'est le cas "
        "par defaut pour toute demande de type 'besoin' (cadeau, occasion) "
        "ou aucune correspondance produit exacte n'existe.\n"
        "   - justification : explique a l'utilisateur POURQUOI ce "
        "commerce repond a SA demande precise, pas une simple reformulation "
        "de la fiche du commerce. Relie explicitement un element de la "
        "demande de l'utilisateur (la personne destinataire, l'occasion, "
        "la contrainte exprimee) a ce que propose le commerce. Mauvais "
        "exemple (reformulation plate de la fiche) : 'Librairie generaliste "
        "proposant livres, BD et papeterie, ideale pour des cadeaux "
        "culturels.' Bon exemple (raisonnement relie a la demande) : "
        "'Pour un cadeau de Noel, cette librairie propose un large choix "
        "de BD et romans jeunesse, une valeur sure si vous ne connaissez "
        "pas les gouts precis du destinataire.' Si confiance='deduit', le "
        "dire explicitement dans la justification ('a confirmer', "
        "'generalement') - ne jamais cacher l'incertitude derriere une "
        ...
        "5. REGLE ABSOLUE : UN COMMERCE NE PEUT JAMAIS APPARAITRE DANS "
        "PLUSIEURS PISTES. Chaque candidat n'est cite qu'UNE SEULE fois "
        "dans toute ta reponse, dans la piste ou il est le plus pertinent. "
        "NE REPETE JAMAIS le meme commerce dans une deuxieme piste avec "
        "une justification differente ou plus forcee pour 'remplir' "
        "artificiellement plusieurs angles - c'est une erreur grave, pas "
        "un detail. Si respecter cette regle te laisse avec moins de "
        "pistes que prevu, ou une seule piste, c'est le comportement "
        "correct, pas un probleme a corriger en dupliquant.\n"
        "6. REGLE ABSOLUE SUR LES EXCLUSIONS EXPLICITES : si la demande de "
